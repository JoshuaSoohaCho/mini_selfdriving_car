...
void CAN_init(void) { // switch to pb8 and pb9 for can and honestly
	// first 37 lines can be initialized in SystemInit, so we can remove them from here
...
//	RCC->AHB2ENR |= GPIOBEN; // Enable clock for GPIOA
	// RX GPIO to pull up
	...
	RCC->APB1ENR1 |= CAN1EN; // Enable clock for CAN1
